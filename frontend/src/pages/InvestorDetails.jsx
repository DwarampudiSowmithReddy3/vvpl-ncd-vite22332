...
const InvestorDetails = () => {
  ...
  const [loadingInvestmentDocs, setLoadingInvestmentDocs] = useState(false);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const updateData = {
        ...
        nominee_email: editFormData.nomineeEmail || undefined,
      ...
      // Upload/Update documents if any were selected
      const documentMapping = {
        panDocument: 'pan_document',
        aadhaarDocument: 'aadhaar_document',
        cancelledCheque: 'cancelled_cheque',
        form15G15H: 'form_15g_15h',
        ...
        <div className="investor-content">
          ...
          <div className="bottom-sections">
            ...
            <div className="section-card">
              ...
              {loadingInvestmentDocs ? (
                <div className="loading-state">Loading investment documents...</div>
                  ...
                  {investmentDocuments.map((doc, index) => (
                    <div key={index} className="document-item">
                      <div className="document-info">
                        <HiOutlineDocumentText className="document-icon" />
                        <div>
                          <div className="document-name">{doc.series_name} - Payment Document</div>
                          <div className="document-date">
                            Investment: ₹{doc.investment_amount?.toLocaleString('en-IN')} • {doc.investment_date}
                          </div>
                          {doc.file_name && (
                            <div className="document-filename">{doc.file_name}</div>
                          )}
                          {doc.uploaded_by && (
                            <div className="document-uploaded-by">Uploaded by: {doc.uploaded_by}</div>
                          )}
                        </div>
                      </div>
                      <div className="document-actions">
                ...
                <div className="empty-state">
                  <p>No investment documents uploaded</p>
                  <span>Payment documents will appear here when investments are made</span>
                </div>
              )}
            </div>
