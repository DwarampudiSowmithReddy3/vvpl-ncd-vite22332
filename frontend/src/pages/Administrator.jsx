...
const Administrator = () => {
  ...
  const [displayedUsers, setDisplayedUsers] = useState([]); // For limited display
  const [totalActiveUsersCount, setTotalActiveUsersCount] = useState(0);
  const [loading, setLoading] = useState(true); // Start with true to show loader on mount
  ...
  const loadUsers = async (searchQuery = null, limitCount = null) => {
    try {
      ...
      if (searchQuery) {
        ...
        setTotalActiveUsersCount(mappedUsers.length);
        // Also set displayed users (first 7)
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      loadUsers(searchTerm, null); // Search with no limit
    }, 300); // 300ms debounce
  ...
  useEffect(() => {
    if (showAllUsers) {
      loadUsers(null, null); // Load all users
    } else {
      loadUsers(null, 7); // Load only 7 users
    }
  }, [showAllUsers]);
