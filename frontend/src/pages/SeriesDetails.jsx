...
const SeriesDetails = () => {
  ...
  // NEW: State for series documents (Term Sheet, Offer Document, Board Resolution)
  // ALL DATA FROM BACKEND - NO FRONTEND LOGIC
  const [seriesDocuments, setSeriesDocuments] = useState(null);
  const [loadingSeriesDocuments, setLoadingSeriesDocuments] = useState(false);
  ...
  // NEW: Fetch series documents from backend (Term Sheet, Offer Document, Board Resolution)
  // ALL DATA FROM BACKEND - Signed URLs generated with 5-minute expiry
  React.useEffect(() => {
    const fetchSeriesDocuments = async () => {
      if (!id) return;
    ...
    <Layout>
      <div className="series-details-page">
        ...
        <div className="documents-section">
          <h2 className="section-title">Series Documents</h2>
          <div className="documents-table-card">
            {loadingSeriesDocuments ? (
              <div className="loading-spinner">Loading documents...</div>
            ) : seriesDocuments && seriesDocuments.documents && seriesDocuments.documents.length > 0 ? (
              <table className="documents-table">
                ...
                <tbody>
                  {seriesDocuments.documents.map((doc) => (
                    <tr key={doc.id}>
                      <td>
              ...
              <div className="no-documents">
                ...
                <p className="draft-message">Series documents (Term Sheet, Offer Document, Board Resolution) will appear here once uploaded.</p>
              </div>
            )}
          </div>
        </div>
