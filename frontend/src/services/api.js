...
class ApiService {
  ...
  async login(username, password) {
    try {
      // Normalize username to avoid common input issues (extra spaces)
  ...
  async updateInvestorDocument(investorId, documentType, file) {
    const formData = new FormData();
    formData.append('file', file);
    
    const url = `${API_BASE_URL}/investors/${investorId}/documents/${documentType}`;
    const currentToken = localStorage.getItem('authToken') || this.token;
  ...
  async uploadComplianceDocument(seriesId, documentTitle, category, description, file) {
    const formData = new FormData();
    ...
    const url = `${API_BASE_URL}/compliance/documents/upload`;
    const currentToken = localStorage.getItem('authToken') || this.token;
