...
# Get the backend root directory (3 levels up from app/services/storage/s3_service.py)
# app/services/storage/s3_service.py -> app/services/storage -> app/services -> app -> backend
...
class S3Service:
    ...
    def validate_file(self, file_name: str, file_size: int, file_content: bytes = None) -> Tuple[bool, str]:
        """
        Validate file type, size, and explicit file signature (magic bytes)
        
        Args:
            file_name: Name of the file
            file_size: Size of file in bytes
            file_content: The actual bytes of the file to verify its signature
        ...
        if file_ext not in ALLOWED_EXTENSIONS:
            return False, f"Invalid file type. Allowed: {', '.join(ALLOWED_EXTENSIONS)}. Got: {file_ext}"
        
        # Check explicit file signature (Magic Bytes) to stop executable disguised as files
        if file_content is not None:
            if file_ext == '.pdf':
                if not file_content.startswith(b'%PDF-'):
                    logger.error(f"❌ SECURITY ALERT: File '{file_name}' does not have a valid PDF signature.")
                    return False, "Security Error: File content does not match a valid PDF document."
            elif file_ext in {'.jpg', '.jpeg'}:
                if not file_content.startswith(b'\xff\xd8\xff'):
                    logger.error(f"❌ SECURITY ALERT: File '{file_name}' does not have a valid JPEG signature.")
                    return False, "Security Error: File content does not match a valid JPEG image."
            elif file_ext == '.png':
                if not file_content.startswith(b'\x89PNG\r\n\x1a\n'):
                    logger.error(f"❌ SECURITY ALERT: File '{file_name}' does not have a valid PNG signature.")
                    return False, "Security Error: File content does not match a valid PNG image."
    ...
    def upload_series_document(
        ...
        """
        try:
            # Validate file size and content signature
            file_size = len(file_content)
            is_valid, error_msg = self.validate_file(file_name, file_size, file_content)
            if not is_valid:
                logger.error(f"❌ File validation failed: {error_msg}")
                return False, None, error_msg
            
            # Generate S3 key (path)
            # Standardized structure: series/{series_id}/{file_name}
            s3_key = f"series/{series_id}/{file_name}"
            ...
            # SECURITY: Return only s3_key and bucket, NOT signed URL
    ...
    def upload_investor_document(
        ...
        try:
            ...
            # Generate S3 key (path)
            # Standardized structure: users/{investor_id}/kyc/{file_name} OR users/{investor_id}/series_{series_id}/{file_name}
            if series_id == 0:
                s3_key = f"users/{investor_id}/kyc/{file_name}"
            else:
                s3_key = f"users/{investor_id}/series_{series_id}/{file_name}"
