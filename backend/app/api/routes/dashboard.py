...
async def get_payout_stats(
    ...
    Returns:
    - total_interest_paid: Sum of ALL paid interest payouts till date (lifetime total)
    ...
    - total_payouts: Count of all paid payouts till date
    ...
    try:
        ...
        # 1. Get TOTAL INTEREST PAID TILL DATE (all paid payouts)
        # This is the SUM of all interest payouts that have been marked as 'Paid'
        logger.info(f"   Calculating total interest paid till date...")
        
        try:
            # Query to get SUM of all PAID interest payouts
            paid_query = """
            SELECT 
            ...
            paid_result = db.execute_query(paid_query)
            
            if paid_result and len(paid_result) > 0:
                total_interest_paid = float(paid_result[0]['total_paid'])
                total_payouts = paid_result[0]['total_count']
            else:
                total_interest_paid = 0.0
                total_payouts = 0
            
            logger.info(f"   Total interest paid till date: ₹{total_interest_paid:,.2f} ({total_payouts} payouts)")
            
        except Exception as e:
            logger.error(f"❌ Error calculating total interest paid: {e}")
            import traceback
            logger.error(traceback.format_exc())
        ...
        try:
            ...
            from app.api.routes.payouts import get_export_payouts
