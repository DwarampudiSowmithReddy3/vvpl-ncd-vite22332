...
from app.utils.series_status_updater import update_series_status_by_dates
...
from app.utils.series_utils import (
    format_currency,
    format_date,
...
async def get_series_upcoming_payouts(
    ...
    try:
        ...
        from app.api.routes.payouts import (
...
async def get_series_recent_payouts(
    ...
    Shows ALL paid payouts from the interest_payouts table (not just current month)
    Returns actual paid transactions with timestamps
    
    Optional date range filters:
    - from_date: Start date (YYYY-MM-DD format) - filters by updated_at (when marked as paid)
    - to_date: End date (YYYY-MM-DD format) - filters by updated_at (when marked as paid)
    
    If no dates provided, returns most recent payouts up to limit
    """
    try:
        db = get_db()
        ...
        if from_date:
            try:
                ...
                logger.info(f"📅 From date filter: {from_date_obj}")
            except ValueError:
                raise HTTPException(
                    status_code=400,
        ...
        if to_date:
            try:
                ...
                # Set to end of day
                to_date_obj = datetime.combine(to_date_obj, datetime.max.time()).date()
                logger.info(f"📅 To date filter: {to_date_obj}")
            except ValueError:
                raise HTTPException(
                    status_code=400,
        ...
        logger.info(f"📅 Fetching recent PAID payouts for series {series_id}")
        ...
        if from_date_obj:
            payouts_query += " AND DATE(ip.updated_at) >= %s"
        ...
        payouts_query += """
        ORDER BY ip.updated_at DESC, ip.payout_date DESC
        LIMIT %s
        """
        
        params.append(limit)
        
        payouts_result = db.execute_query(payouts_query, tuple(params))
        
        # Format the results
        payouts = []
        for row in payouts_result:
            # Use updated_at as the paid timestamp (when status was changed to Paid)
            paid_timestamp = row['updated_at'] if row['updated_at'] else row['created_at']
            
            if paid_timestamp:
                paid_date_str = paid_timestamp.strftime('%d-%b-%Y')
                paid_time_str = paid_timestamp.strftime('%d/%m/%Y %H:%M:%S')
            else:
                # Fallback to payout_date if no timestamp
                payout_date = row['payout_date']
                if isinstance(payout_date, str):
                    paid_date_str = payout_date
                else:
                    paid_date_str = payout_date.strftime('%d-%b-%Y') if payout_date else 'N/A'
                paid_time_str = None
            ...
            payouts.append({
                ...
                'payout_date': paid_date_str,
                'amount': float(row['amount']),
                'status': row['status'],
                'paid_date': paid_time_str,
                'created_at': row['created_at'].strftime('%d/%m/%Y %H:%M:%S') if row['created_at'] else None
            })
...
async def approve_series(
    ...
    try:
        ...
        from app.utils.date_utils import parse_date_flexible, format_date_for_db
    ...
    try:
        ...
        for doc_type, file in documents:
            if file:
                ...
                # Save to database - SECURITY: Store only s3_key, NOT signed URL
                # Signed URLs will be generated dynamically when needed
                insert_query = """
                INSERT INTO series_documents (
                    series_id, document_type, file_name, s3_bucket, s3_key,
...
async def get_series_documents(
    ...
    try:
        ...
        for doc in result:
            ...
            # Even if URL is stolen, it becomes useless after 5 minutes
