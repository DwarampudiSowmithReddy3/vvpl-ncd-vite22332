...
async def get_investor(
    ...
    try:
        ...
        # Get documents - CRITICAL: Use database ID, not investor_id string
        # Documents are stored with the integer database ID
        docs_query = "SELECT * FROM investor_documents WHERE investor_id = %s"
        docs_result = db.execute_query(docs_query, (db_id,))
        documents = []
        ...
        for doc in docs_result:
            # SECURITY: Generate fresh signed URL with 5-minute expiry
            # URL expires in 300 seconds (5 minutes) for maximum security
            download_url = None
            if doc.get('file_path'):  # file_path contains s3_key
                download_url = s3_service.generate_signed_url(
            ...
            documents.append({
                ...
                "download_url": download_url  # Fresh signed URL (expires in 5 min)
            })
        ...
        for doc in documents:
            kyc_documents.append({
                ...
                "download_url": doc['download_url']  # Fresh signed URL (expires in 5 min)
            })
        ...
        for inv in investments:
            if inv['status'] == 'active':
                ...
                if series_detail and len(series_detail) > 0:
                    ...
                    # Calculate next payout date based on interest_payment_day from ncd_series
                    next_payout = 'N/A'
                    try:
                        # Get interest_payment_day from series
                        payment_day_query = """
                        SELECT interest_payment_day
                        FROM ncd_series
                        WHERE id = %s
                        """
                        payment_day_result = db.execute_query(payment_day_query, (inv['series_id'],))
                        
                        if payment_day_result and payment_day_result[0]['interest_payment_day']:
                            payment_day = payment_day_result[0]['interest_payment_day']
                            ...
                            # If today's date is before payment day this month, next payout is this month
                            # Otherwise, next payout is next month
                            if today.day < payment_day:
                                # Next payout is this month
                                next_payout_month = today.month
                                next_payout_year = today.year
                            else:
                                # Next payout is next month
                                next_payout_month = today.month + 1
                                next_payout_year = today.year
                                if next_payout_month > 12:
                                    next_payout_month = 1
                                    next_payout_year += 1
                            
                            # Handle case where payment_day exceeds days in month (e.g., Feb 30)
                            days_in_month = monthrange(next_payout_year, next_payout_month)[1]
                            actual_payment_day = min(payment_day, days_in_month)
                            
                            next_payout_date = date(next_payout_year, next_payout_month, actual_payment_day)
                            next_payout = next_payout_date.strftime('%d-%b-%Y')
                            
                            logger.info(f"✅ Calculated next payout: {next_payout} (payment day: {payment_day})")
                        else:
                            logger.info(f"⚠️ No interest_payment_day found for series {inv['series_id']}")
                    except Exception as e:
                        logger.error(f"❌ Error calculating next payout: {e}")
                        import traceback
...
@router.get("/{investor_id}/investment-documents")
async def get_investor_investment_documents(
    investor_id: str,
    current_user: dict = Depends(get_current_user),
    db = Depends(get_db)
):
    """
    Get investment payment documents for an investor
    Returns documents with fresh signed URLs (5-minute expiry)
    """
    try:
        # Get investor by investor_id (string like "INV001")
        investor_query = "SELECT id, investor_id, full_name FROM investors WHERE investor_id = %s"
        investor_result = db.execute_query(investor_query, (investor_id,))
        
        if not investor_result:
            raise HTTPException(status_code=404, detail="Investor not found")
        
        db_id = investor_result[0]['id']
        
        # Get investment documents with series information
        query = """
        SELECT 
            iid.id,
            iid.investment_id,
            ...
            i.date_received as investment_date
        FROM investor_investment_documents iid
        LEFT JOIN ncd_series s ON iid.series_id = s.id
        LEFT JOIN investments i ON iid.investment_id = i.id
        WHERE iid.investor_id = %s AND iid.is_active = 1
        ORDER BY iid.uploaded_at DESC
        """
        
        result = db.execute_query(query, (db_id,))
        
        if not result:
            return {
                "investor_id": investor_id,
                "documents": [],
                "message": "No investment documents found"
            }
        ...
        for doc in result:
            ...
            documents.append({
                ...
                'investment_amount': float(doc['investment_amount']) if doc['investment_amount'] else 0.0,
                'investment_date': doc['investment_date'].strftime('%d/%m/%Y') if doc['investment_date'] else 'N/A',
                'document_type': doc['document_type'],
                'file_name': doc['file_name'],
        ...
        logger.error(f"Error fetching investment documents: {e}")
        import traceback
        logger.error(traceback.format_exc())
        raise HTTPException(status_code=500, detail=str(e))
...
async def add_investment(
    ...
    try:
        ...
        payment_document_s3_key = None
        payment_document_s3_bucket = None
        investment_doc_id = None
        ...
        if payment_document and payment_document.filename:
            try:
                ...
                if s3_service is None:
                    ...
                    # Generate S3 key: users/{investor_id}/investments/{series_code}/payment_{timestamp}.{ext}
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    file_extension = payment_document.filename.split('.')[-1] if '.' in payment_document.filename else 'pdf'
                    s3_key = f"users/{investor_data['investor_id']}/investments/{series_data['series_code']}/payment_{timestamp}.{file_extension}"
                    
                    # Get content type
                    import mimetypes
                    content_type = mimetypes.guess_type(payment_document.filename)[0] or 'application/pdf'
                    
                    # Upload to S3 using s3_client directly
                    logger.info(f"📤 Uploading payment document to S3: {s3_key}")
                    ...
                    s3_service.s3_client.put_object(
                        ...
                        Metadata={
                            'investor_id': str(investor_data['investor_id']),
                            'series_id': str(series_id),
                            'document_type': 'payment_document',
                            'original_filename': payment_document.filename,
                            'uploaded_at': datetime.utcnow().isoformat()
                        }
                    )
                    
                    # SECURITY: Store only s3_key and bucket, NOT signed URL
                    payment_document_s3_key = s3_key
                    payment_document_s3_bucket = s3_service.bucket_name
        ...
        # Insert investment - store only s3_key, NOT full URL
        insert_query = """
        ...
        db.execute_query(insert_query, (
            ...
            payment_document_s3_key  # Store only s3_key, not full URL
        ))
        
        # Get the created investment ID
        get_investment_id_query = "SELECT LAST_INSERT_ID() as id"
        investment_result = db.execute_query(get_investment_id_query)
        investment_id = investment_result[0]['id'] if investment_result else None
        
        # Save payment document to investor_investment_documents table (separate from KYC docs)
        if payment_document_s3_key and investment_id:
            try:
                doc_insert_query = """
                INSERT INTO investor_investment_documents (
                    investment_id, investor_id, series_id, document_type, 
                    file_name, file_path, s3_bucket, file_size, content_type,
                    uploaded_by
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """
                
                db.execute_query(doc_insert_query, (
                    investment_id,
                    investor_id,
                    series_id,
                    'payment_document',
                    payment_document.filename,
                    payment_document_s3_key,  # file_path stores s3_key
                    payment_document_s3_bucket,
                    file_size,
                    content_type,
                    current_user.full_name
                ))
                
                logger.info(f"✅ Payment document record saved to investor_investment_documents table")
                
            except Exception as e:
                logger.error(f"❌ Error saving payment document record: {e}")
        ...
        create_audit_log(
            ...
            changes={
                ...
                "payment_document_uploaded": payment_document_s3_key is not None,
                "status": "confirmed",
...
async def upload_series_investor_document(
    ...
    try:
        ...
        # Save document record to database - SECURITY: Store only s3_key, NOT signed URL
        # Signed URLs will be generated dynamically when needed (5-minute expiry)
        insert_query = """
        INSERT INTO investor_documents (
            investor_id, document_type, file_name, file_path, file_size,
            s3_bucket, content_type
        ...
        db.execute_query(insert_query, (
            ...
            doc_info.get('s3_key'),
            doc_info['file_size'],
            doc_info.get('s3_bucket'),
            doc_info.get('content_type')
        ))
...
@router.put("/{investor_id}/documents/{document_type}", response_model=InvestorDocumentResponse)
async def update_investor_document(
    investor_id: int,
    document_type: str,
    file: UploadFile = File(...),
    request: Request = None,
    ...
    """
    Update/Replace existing KYC document for investor
    - Deletes old document from S3
    - Uploads new document to S3
    ...
    try:
        ...
        check_query = """
        SELECT id, file_path, s3_bucket FROM investor_documents
        WHERE investor_id = %s AND document_type = %s
        """
        existing_doc = db.execute_query(check_query, (investor_id, document_type))
        ...
        # If old document exists, delete it from S3
        if existing_doc and existing_doc[0].get('file_path'):
            old_s3_key = existing_doc[0]['file_path']
            logger.info(f"🗑️ Deleting old document from S3: {old_s3_key}")
            try:
                s3_service.delete_file(old_s3_key)
            except Exception as e:
                logger.warning(f"⚠️ Failed to delete old document from S3: {e}")
                # Continue anyway - we'll overwrite the database record
        ...
        # Update or insert document record - SECURITY: Store only s3_key and s3_bucket
        if existing_doc:
            # Update existing record
            update_query = """
            UPDATE investor_documents
            SET file_name = %s, file_path = %s, file_size = %s, 
                s3_url = NULL, s3_bucket = %s, content_type = %s, uploaded_at = NOW()
            WHERE investor_id = %s AND document_type = %s
            """
            ...
            # Insert new record
            insert_query = """
            INSERT INTO investor_documents (investor_id, document_type, file_name, file_path, file_size, s3_url, s3_bucket, content_type, uploaded_at)
            ...
            action_verb = "Uploaded"
        
        # Get the updated document
        select_query = "SELECT * FROM investor_documents WHERE id = %s"
        result = db.execute_query(select_query, (doc_id,))
        ...
        create_audit_log(
            db=db,
            action=f"{action_verb} Investor Document",
            admin_name=current_user.full_name,
            admin_role=current_user.role,
            details=f"{action_verb} {document_type.replace('_', ' ').title()} document for investor \"{investor_data['full_name']}\" (ID: {investor_data['investor_id']}) - File: {file.filename} ({file_size / 1024:.2f} KB)",
            entity_type="Document",
            entity_id=investor_data['investor_id'],
            changes={
                "investor_id": investor_data['investor_id'],
                ...
                "file_name": file.filename,
                "file_size": file_size,
                "action": "document_update" if existing_doc else "document_upload"
            },
        ...
        doc_data['download_url'] = s3_service.generate_signed_url(doc_data['file_path'], expiry=300)
        
        logger.info(f"✅ {action_verb} {document_type} for investor {investor_id}")
        return InvestorDocumentResponse(**doc_data)
