...
def calculate_monthly_interest(principal_amount: float, annual_interest_rate: float, month: int, year: int) -> float:
    """
    Calculate interest for REGULAR FULL MONTH based on ACTUAL DAYS in that month
    
    NEW LOGIC (Days-based):
    - Calculate: (Principal × Rate% × Days_in_Month) / Days_in_Year
    - Each month gets DIFFERENT amount based on actual days
    - Uses Python calendar module for accuracy
    
    Formula: (Principal × Rate% × Days_in_Month) / Days_in_Year
    
    Examples:
    - January (31 days): ₹10,00,000 × 12% × 31/365 = ₹10,191.78
    - February (28 days): ₹10,00,000 × 12% × 28/365 = ₹9,205.48
    - February (29 days - leap): ₹10,00,000 × 12% × 29/366 = ₹9,508.20
    - March (31 days): ₹10,00,000 × 12% × 31/365 = ₹10,191.78
    - April (30 days): ₹10,00,000 × 12% × 30/365 = ₹9,863.01
    
    Annual total remains: ₹1,20,000 (12% of principal)
    ...
    try:
        ...
        # Get actual days in the month using calendar module
        days_in_month = calendar.monthrange(year, month)[1]
        
        # Determine if leap year using calendar module
...
def calculate_first_month_interest(
    ...
    """
    Calculate interest for FIRST month based on ACTUAL DAYS
    
    NEW LOGIC (Always days-based):
    - Calculate from series start date to last day of that month
    - Uses actual days regardless of whether it starts on 1st or mid-month
    
    Formula: (Principal × Rate% × Days) / Days_in_Year
    
    Where Days = From series start date to LAST DAY of that month
    
    Examples:
    1. Series starts Feb 1:
       - Calculate: Feb 1 to Feb 28 = 28 days
       - Interest: (10,00,000 × 12% × 28) / 365 = ₹9,205.48
    
    2. Series starts Feb 2:
       - Calculate: Feb 2 to Feb 28 = 27 days
       - Interest: (10,00,000 × 12% × 27) / 365 = ₹8,876.71
    
    3. Series starts Feb 15:
       - Calculate: Feb 15 to Feb 28 = 14 days
       - Interest: (10,00,000 × 12% × 14) / 365 = ₹4,602.74
...
def calculate_exit_interest(
    ...
    """
    Calculate interest for EXIT based on ACTUAL DAYS
    
    NEW LOGIC (Always days-based):
    - Calculate from 1st of exit month to exit date
    - Uses actual days regardless of whether exit is on last day or mid-month
    ...
    try:
        ...
        # Calculate from start of exit month to exit date
...
def calculate_maturity_interest(
    ...
    """
    Calculate interest for MATURITY based on ACTUAL DAYS
    
    NEW LOGIC (Always days-based):
    - Calculate from 1st of maturity month to maturity date
    - Uses actual days regardless of whether maturity is on last day or mid-month
    ...
    try:
        ...
        # Calculate from start of maturity month to maturity date
...
async def get_all_payouts(
    ...
    try:
        ...
        # RULE 4: Show payouts for ANY series that has started (series_start_date <= CURDATE())
        #         regardless of series status (DRAFT, upcoming, accepting, active, matured, etc.)
        # RULE 5: Exclude investments where maturity date is BEFORE the current payout month
        query = """
        SELECT 
        ...
        params = [current_month_number, current_month_number]
...
async def get_export_payouts(
    ...
    try:
        ...
        # RULE 4: Show payouts for ANY series that has started (series_start_date <= CURDATE())
        #         regardless of series status (DRAFT, upcoming, accepting, active, matured, etc.)
        # RULE 5: Exclude investments where maturity date is BEFORE the target payout month
        query = """
        SELECT 
            inv.id as investor_id,
            inv.investor_id as investor_code,
        ...
        params = [target_month_number, target_month_number]
